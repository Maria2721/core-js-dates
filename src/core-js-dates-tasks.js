...
function getTime(date) {
  ...
  const fullStr = (value) => String(value).padStart(2, '0');

  return `${fullStr(hour)}:${fullStr(minutes)}:${fullStr(seconds)}`;
}
...
function getNextFriday(date) {
  ...
  const daysNextFriday = day === 5 ? 7 : (5 - day + 7) % 7;
  result.setUTCDate(result.getUTCDate() + daysNextFriday);
...
function isDateInPeriod(date, period) {
  ...
  return start <= result.valueOf() && end >= result.valueOf();
}
...
function getWeekNumberByDate(date) {
  ...
  const dayWeek = dateUTC.getUTCDay();
  const dayWeekNum = dayWeek === 0 ? 7 : dayWeek;
  dateUTC.setUTCDate(dateUTC.getUTCDate() + 4 - dayWeekNum);

  const start = new Date(Date.UTC(dateUTC.getUTCFullYear(), 0, 1));
  const oneDay = 24 * 60 * 60 * 1000;

  return Math.ceil(((dateUTC - start) / oneDay + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  const curDate = new Date(date.getFullYear(), date.getMonth(), 13);
  const limitYear = date.getFullYear() + 10;

  while (curDate.getFullYear() < limitYear) {
    if (curDate > date && curDate.getDay() === 5) return curDate;
...
function parseDateSchedule(str) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const start = parseDateSchedule(period.start);
  const end = parseDateSchedule(period.end);
  const schedule = [];

  while (start <= end) {
    for (let i = 0; i < countWorkDays && start <= end; i += 1) {
      schedule.push(formatDateSchedule(start));
